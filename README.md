...
Considering that the static website doesn't change frequently, it is not necessary to deploy the website itself everytime. Instead of deploying the whole api-doc everytime, uploading the generated swagger.json to S3 to update the API specification by CI/CD is much more efficient. (Only need to deploy the API Doc when there is any change in Swagger UI static source code.)
...
- Add a API Doc Serverless service
  ...
  - Or you can create a new one:
    ...
    - use a custom plugin `deployment/plugins/s3-upload-plugin.js` to deploy static website
    - use variables defined in `go-clean-arch-lambda-api/configs` to create serverless resource. check `deployment/db/serverless.yml` for detail
- Deploy the created API Doc Serverless service
  - open root folder
  - make sure the required packages are installed
    - should be installed by `npm install`
    - if not installed, run cmd `npm install mine --save-dev`
  - run cmd `npm run build:apidoc`
  - run cmd `npm run package:apidoc`
  - run cmd `npm run deploy:apidoc` to prepare and deploy static resources
  - if you have a aws profile, change deploy cmd in `package.json` as 
  ```
  "deploy:apidoc": "cd deployment/api-doc && sls upload --aws-profile {your profile name}"
  ````
- Update Swagger specification from local
  - run cmd `npm run swag` to generate swagger spec
  - directly upload the new swagger spec to S3 bucket manually
